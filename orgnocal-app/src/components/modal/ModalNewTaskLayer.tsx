...
import { useCreateLayerMutation, TaskLayer, useUpdateTaskLayerNameMutation } from "@/state/api";
...
const ModalCreateEditTaskLayer = ({ isOpen, onClose, editingModalLayerId, taskLayers, projectId }: ModalCreateEditTaskLayerProps) => {
  const [createLayer, { isLoading: isLoadingCreating }] = useCreateLayerMutation();
  const [editLayer, { isLoading: isLoadingEditing }] = useUpdateTaskLayerNameMutation();
  // TODO: If getting to a lot validation use library
  const [name, setName] = useState("");
  const currentLayer = taskLayers?.find((layer) => layer.id === editingModalLayerId)
  ...
  const handleSubmit = async () => {
    if (!name || (taskLayers && taskLayers.length >= 5)) return;
    ...
    if (!editingModalLayerId) {
      ...
      if (currentLayer && currentLayer.tasks.length > 0) {
        await editLayer({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} name="Create New Task Layer">
      <form
      ...
      >
        ...
        >
          {/* TODO: Add spinner */}
          {invalidateSubmit() ? "Creating Layer . . ." : "Create Layer"}
        </button>
      </form>
    </Modal>
  );
